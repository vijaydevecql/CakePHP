			...
			<div class="container">
										...
										<tr id="cart_row_<?php echo $value['id']; ?>">
											...
											<td class="quty" id="quantity_<?php echo $value['id']; ?>">
													...
													id="total_product_price_<?php echo $value['id']; ?>">
														...
														<a href="javascript:void(0);" class="cros">
				...
				?>
					...
					<div class="col-md-4 col-md-offset-2">
						...
						<div class="new_cart"> <h3> Promo Code : <?php echo @$_pro_name; ?> <span id="p_a" ></span> </h3> <h4> <span id="dis_promo_total">$<?php echo !empty($sub_total)?number_format($p_amount,2):0.00; ?> </span> <span class="items_text" id="rm_promo"><?php if($is_use){ ?>
						<a href="<?php echo $this->webroot.'remove_coupon';?>"><i class="fa fa-trash green"></i></a><?php }?> </span> </h4></div>
...
$('body').on('click', '.cross', function (e) {
				...
				//return false;
					...
					if (html === '0') {
						...
						Subtotal = parseInt(Subtotal) -  parseInt(p_price); //Total
						
						//alert("1"+Subtotal);
						
						total = parseInt(total) -  parseInt(p_price); //subtotal
						//alert("2"+total);

						var promo_per = $("#promoa").data('promoa');
						
						var cal_per_amount = (parseInt(total)*parseInt(promo_per))/100;
						//alert("3"+cal_per_amount);
						
						var Subtotal_after_promo_deduct = parseInt(total) - parseInt(cal_per_amount);
						//alert("4"+Subtotal_after_promo_deduct);

						Subtotal = Subtotal_after_promo_deduct;
					
						//Subtotal = Subtotal_after_promo_deduct;
					
						//alert(total + 'subtotal after delete pro' );

						Subtotal = Subtotal.toFixed(2);
						//alert(Subtotal + 'total after tofixed ');
						total = total.toFixed(2);
						cal_per_amount = cal_per_amount.toFixed(2);
						//alert(total + 'subtotal after to fixed');


						$("#subtotal").attr('data-price',Subtotal);
						$("#dis_sub_total").html('$'+Subtotal);
						$("#dis_promo_total").html('$'+cal_per_amount);
