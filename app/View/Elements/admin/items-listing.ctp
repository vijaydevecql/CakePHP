
<div class="page-header">
    <h1 class="page-title">
		Requests
    </h1>
    <br />
</div>
<div class="panel">
    <div class="panel-body container-fluid">
        <div class="row row-lg">
            <div class="col-md-12">
                <!-- Example Basic -->
                <div class="example-wrap">
                    <div class="example table-responsive">
                        <table class="table">
                            <thead>
                                <tr>
                                    <th>Id</th>
                                    <th>Product</th>
                                    <th>Borrowed User</th>
                                    <th>Lended User</th>
									<th>Status</th>
									<th>Action</th>
                                </tr>
                            </thead>
                            <tbody>
								<?php
								if (isset($borrow_lend) && count($borrow_lend)) {
									foreach ($borrow_lend as $req) {
										?>
										<tr>
											<td><?php echo $req['BorrowLend']['id']; ?></td>
											<td>
												<?php
                                                $image = 'default_product_photo.jpg';
                                                if(!empty($req['Item']['photo'])){
                                                     
                                                    $path = APP . 'webroot' . DS . 'files' . DS . 'items_photo'. DS . $req['Item']['photo'];
                                                    if(file_exists($path)){
                                                        $image = '100x100'.$req['Item']['photo'];
                                                    }
                                                }
                                            ?>
                                            <img src="<?php echo $this->webroot.'files/items_photo/'.$image;?>"  class="user_photo">
											</td>
											<td>
												<a href="<?php echo $this->webroot . 'admin/users/view/' . $req['User']['id'] ?>">
												<?php echo isset($req['User']['email'])?ucfirst($req['User']['email']):'--'; ?></a>
											</td>
											<td>
											<?php if(count($req['Item']['User'])){ ?>
												<a href="<?php echo $this->webroot . 'admin/users/view/' . $req['User']['id'] ?>">
												<?php echo $req['Item']['User']['email']; ?></a>
											<?php }else { 
												echo '--';
											}
											?>

												
											</td>
											
											<td class="update_status">
												<?php
												echo $req['BorrowLend']['status'] == 1 ?
														$this->Html->link(
																'Active', '/borrowLends/admin_update_status/' . $req['BorrowLend']['id'], array(
															'class' => 'label label-success',
															'rel' => $req['BorrowLend']['id']
																)
														) :
														$this->Html->link(
																'Inactive', '/borrowLends/admin_update_status/' . $req['BorrowLend']['id'], array(
															'class' => 'label label-danger',
															'rel' => $req['BorrowLend']['id']
																)
														)
												;
												?>
											</td>
											<td>
												<?php
													echo $this->Html->link(
															'View', '/admin/request/view/' . $req['BorrowLend']['id'], array(
														'class' => 'label label-info',
														'rel' => $req['BorrowLend']['id']
															)
													);
												?>
											</td>
										</tr>
		<?php
	}
} else {
	?>
									<tr>
										<td colspan = "4" class="text-center">
											<span>No Record Found</span>
										</td>
									</tr>
<?php } ?>

                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

<script>
	$(document).ready(function () {
		$(document.body).on('click', '.update_status a', function (e) {
			e.preventDefault();
			_this = $(this);
			_id = _this.attr('rel');
			_this.addClass('disabled');
			$.ajax({
				url: "<?php echo $this->webroot . 'admin/borrowLends/update_status/' ?>" + _id,
				cache: false
			}).done(function (html) {
				if (html === '1') {
					_this.removeClass('label-danger');
					_this.addClass('label-success');
					_this.text('Active');
				}
				if (html === '2') {
					_this.removeClass('label-success');
					_this.addClass('label-danger');
					_this.text('Inactive');
				}
				_this.removeClass('disabled');
			});

		});
		// $('body').on('click', '.del', function () {
		//     var id = $(this).val();
		//     $(".showSweetAlert").find('p').text("lead text-muted");
		//     var table = $(this).attr("msg");
		//     swal({
		//         title: "Are you sure?",
		//         text: "You want to delete this Ads !",
		//         type: "warning",
		//         showCancelButton: true,
		//         confirmButtonClass: "btn-danger",
		//         confirmButtonText: "Yes, delete it!",
		//         cancelButtonText: "No, cancel !",

		//         closeOnConfirm: false,
		//         closeOnCancel: false,
		//         showLoaderOnConfirm: true,
		//     }, function (isConfirm) {
		//         if (isConfirm) {
		//             $.ajax({
		//                 url: "<?php echo $this->webroot . 'delete'; ?>",
		//                 type: "POST",
		//                 data: {
		//                     "id": id,
		//                     "table": table
		//                 },
		//                 dataType: "html",
		//                 success: function ()
		//                 {

		//                     $("#del_" + id).closest("tr").remove();

		//                     //table1.ajax.reload();
		//                     //table.ajax.reload();
		//                     swal("Done!", "It was succesfully deleted!", "success");
		//                 },
		//                 error: function (xhr, ajaxOptions, thrownError) {
		//                     swal("Error deleting!", "Please try again", "error");
		//                 }
		//             });
		//         } else
		//         {
		//             swal("Cancelled", table + " is safe :)", "error");
		//         }
		//     });
		// });
	});
</script>
<?php
echo $this->Element('pagination');
